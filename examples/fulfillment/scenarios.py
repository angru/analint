...
from .actions import (
    authorize_payment, cancel_by_customer, capture_payment, close_lost_order,
    compensate_failed_payment, confirm_delivery, confirm_order, decline_payment,
    dispatch, refund_after_cancel, refund_lost, reject_out_of_stock,
    release_after_cancel, report_lost, reserve_stock, supplier_restock,
)
...
def _world(order=OrderStatus.PLACED, stock=1, res=ReservationStatus.NONE,
           pay=PaymentStatus.NONE, ship=ShipmentStatus.NONE):
    return [
        Order(status=order),
...
sc_reserve = Scenario(
    ...
    then=[Assert(Reservation.status == ReservationStatus.RESERVED),
          Assert(Warehouse.stock == 0)],
)
...
sc_capture = Scenario(
    ...
    given=_world(order=OrderStatus.CONFIRMED, res=ReservationStatus.RESERVED,
...
sc_refund = Scenario(
    ...
    given=_world(order=OrderStatus.CANCELLED, res=ReservationStatus.RESERVED,
                 pay=PaymentStatus.AUTHORIZED),
...
sc_release = Scenario(
    ...
    then=[Assert(Reservation.status == ReservationStatus.RELEASED),
          Assert(Warehouse.stock == 1)],
)
...
sc_dispatch = Scenario(
    ...
    given=_world(order=OrderStatus.CONFIRMED, res=ReservationStatus.RESERVED,
                 pay=PaymentStatus.CAPTURED),
    then=[Assert(Shipment.status == ShipmentStatus.DISPATCHED),
...
sc_delivery = Scenario(
    ...
    given=_world(order=OrderStatus.SHIPPED, res=ReservationStatus.CONSUMED,
                 pay=PaymentStatus.CAPTURED, ship=ShipmentStatus.DISPATCHED),
...
sc_refund_lost = Scenario(
    ...
    action=refund_lost,
    given=_world(order=OrderStatus.SHIPPED, res=ReservationStatus.CONSUMED,
                 pay=PaymentStatus.CAPTURED, ship=ShipmentStatus.LOST),
    then=[Assert(Payment.status == PaymentStatus.REFUNDED)],
)
...
sc_close_lost = Scenario(
    ...
    given=_world(order=OrderStatus.SHIPPED, res=ReservationStatus.CONSUMED,
                 pay=PaymentStatus.REFUNDED, ship=ShipmentStatus.LOST),
    then=[Assert(Order.status == OrderStatus.CANCELLED)],
)
