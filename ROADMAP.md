...
авто-верификацией инвариантов, исполняемые `Flow`. 289 тестов; первая внешняя
...
- ⏳ один и тот же case в analint и Quint или FizzBee — пока структурное
  сравнение (research/23), не полный порт
- ✅ серия изменений требований измерена на первой модели: «require code-owner
  approval» = ~6 строк (политика — один предикат `mergeable`), сразу поймала
  регрессию существующего сценария и пересчитала witness (research/23)
- ✅ первый внешний change-oriented candidate реализован: GitHub protected
  ...
  Вывод: события НЕ понадобились — state-chaining достаточно (подтверждает
  разворот research/22)
