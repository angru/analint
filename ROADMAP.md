...
- ✅ **synthesis verdict (research/25):** the gate is passed and the evidence did
  NOT demand a new verification primitive. The boundaries hit (liveness/history,
  identity multiplicity, cross-cutting refinement) are principled, and where they
  matter Quint/Apalache are strictly stronger. Decision:
  1. **freeze the verification core at bounded reachability** — re-open only on
     *repeated* real-evidence pain, not one case;
  2. **invest in the differentiated layer**, not in re-implementing a checker:
     the agent/authoring surface (`affects`/`--what-if`/exploration artifacts),
     domain readability, scenario/flow diagnostics, spec-as-living-doc;
  3. **offer a Quint export bridge** for "beyond reachability" (the oauth.qnt
     port shows the mapping is mechanical) — realise the "domain frontend over an
     existing verifier" idea as a bridge, keeping analint's fast bounded checker
     for the common case.
