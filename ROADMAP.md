...
в research/17 §3 (журнал решений).

Этот роадмап выведен из исследования в `research/` (файлы 00–17). Краткая
...
## Текущие приоритеты (разворот от 13 июня 2026)
...
проверить её согласованность — без привязки к реализации**. Обоснование и
журнал решений: research/17 §3.

Топ-приоритеты (фаза v1.3 ниже):

1. **Verification by default** — согласованность проверяется без явных queries.
2. **Исполняемый многошаговый сценарий** — оживить `Flow` или ввести
   многошаговый `Scenario`.
3. **Решить судьбу `emits/on`** — честная event-семантика либо явная documentary-аннотация.
4. **Предупреждать о безграничных числовых доменах** до запуска explorer.
...
каком виде», авансом не строим. Дешёвая замена semantic diff — прогнать
верификатор на двух git-ревизиях спеки и сравнить результаты; агент дифает
коммиты самой спеки. Условия возврата наверх — в research/17 §3 («Как откатиться»).
...
### v1.3 — Полнота движка (активная фаза, с 13 июня 2026)

Базовая петля «описать систему → узнать, согласована ли она» должна работать
надёжно и без церемоний. Выведено из эмпирического аудита (research/17 §1).

- **Verification by default** — инварианты (как неявный `AlwaysHolds`),
  достижимость лайфциклов и отсутствие softlock проверяются **без** явных queries.
  Закрывает ложный зелёный: сейчас explorer запускается только при `spec.queries`,
  поэтому спека с инвариантами без queries не проверяется по поведению
  (research/17 §1.7). Высший приоритет.
- **Исполняемый многошаговый сценарий** — `Flow` сейчас не прогоняется через
  состояние (только структурно; research/17 §1.6). Либо оживить `Flow` (post-шага
  → pre-следующего, проверки на каждом), либо ввести многошаговый `Scenario`. Это
  здоровое ядро идеи «слоёв»: чекпоинты, полученные применением **настоящих**
  переходов, без рукописных дельт-состояний (они ломают соундность).
- **Судьба `emits/on`** — `on=[Event]` не гейтит действие в explorer
  (research/17 §1.3). Либо честная семантика event pool (consume/broadcast,
  payload binding, dead-subscription queries), либо явная documentary-аннотация с
  исключением из exploration. Выбрать и зафиксировать.
- **Предупреждение о безграничных числовых доменах** до запуска explorer —
  сейчас они молча усекаются по `max_states` (research/17 §1.2). Дёшево.
- Затем — точечная выразительность по спросу (`Computed`, guards на переходах),
  измеренная на реальной модели.
