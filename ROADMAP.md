...
- ⏳ **серия** изменений требований — измерена пока ОДНА правка («require
  code-owner approval» = ~6 строк, поймала регрессию `sc_merge_happy` и
  пересчитала witness, research/23). Серия (вкл/выкл правил, strict→loose,
  cross-cutting) с сохранёнными diff и стоимостью — остаётся
