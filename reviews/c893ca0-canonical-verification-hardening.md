...
## Re-review after 321ffdc

Повторная проверка: 14 июня 2026.

### Verdict

Все четыре finding этого review закрыты корректно:

- невычислимый `Spec.initial` без consumers теперь даёт общий FAIL;
- foreign/duplicate/unregistered snapshots в `Initial.given` валидируются;
- неположительный `Spec.max_states` отклоняется Pydantic-моделью;
- findings с одинаковым текстом и разными locations больше не схлопываются.

Новых correctness-дефектов в изменённом коде не найдено. Статус P1 можно
считать закрытым.

### P3. Docstring обещает ещё не реализованный reuse для default-source queries

Файлы:

- `src/analint/validator/explorer.py:581`
- `src/analint/validator/explorer.py:523`
- `src/analint/validator/engine.py:116`

`build_canonical_initials()` утверждает:

```text
Built once per run and reused by both invariant verification and
default-source queries.
```

Фактически engine передаёт построенные roots только в `verify_invariants`.
`run_query()` для query без собственного source снова вызывает
`build_initial_relation(spec, spec.initial)`. Это согласуется с последним
абзацем Resolution, где reuse exploration/default-source queries явно отложен,
но противоречит docstring и слишком широкой формулировке «строится один раз».

Это не влияет на корректность: relation детерминирован и обе стороны получают
эквивалентные roots. Нужно либо убрать из docstring обещание reuse queries,
либо передать pre-built canonical roots/cache в query phase. До реализации
оптимизации точнее написать: «built once for canonical validation and invariant
verification».

#### Resolution (P3)

Закрыто: docstring `build_canonical_initials` переписан под фактическое
поведение — «built once per run for canonical-initial validation and invariant
verification», с явной пометкой, что default-source queries пока строят
эквивалентные roots сами (отложенная оптимизация, не correctness). Сам reuse в
query phase не реализован сознательно. 275 passed; ruff/ty зелёные.

### Verification

```text
.venv/bin/pytest -q              275 passed, 1 skipped
.venv/bin/ruff check .           passed
.venv/bin/ruff format --check .  passed
.venv/bin/ty check               passed
git diff c893ca0..HEAD --check   passed
```
...
fulfillment    PASS
sunless_crypt  PASS
coin           expected FAIL (deliberate supply-overflow counterexample)
```
