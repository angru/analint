...
code_owner_approve = Action(
    ...
    pre=[
        is_open,
        Not(PullRequest.code_owner_approved),
    ],
    effect=[
...
dismiss_changes_request = Action(
    ...
    pre=[is_open, PullRequest.changes_requested],
...
base_advanced = Action(
    ...
    pre=[is_open, Not(PullRequest.behind_base)],
...
update_branch = Action(
    ...
    pre=[is_open, PullRequest.behind_base],
...
code_owner_is_an_approval = Invariant(
    Implies(PullRequest.code_owner_approved, PullRequest.approvals >= 1),
...
sc_push_keeps_changes_requested = Scenario(
    ...
    then=[PullRequest.changes_requested],
)
...
sc_dismiss_changes_request = Scenario(
    ...
    given=[PullRequest(changes_requested=True)],
    then=[Not(PullRequest.changes_requested)],
)
...
flow_changes_request_blocks_until_dismissed = Flow(
    ...
    steps=[
        request_changes,
        Assert(PullRequest.changes_requested),
...
flow_push_forces_reapproval = Flow(
    ...
    steps=[
        ...
        Assert(Not(PullRequest.code_owner_approved)),
        approve,
        code_owner_approve,
        checks_pass,
        merge,
...
never_merge_without_code_owner = Unreachable(
    And(PullRequest.state == PRState.MERGED, Not(PullRequest.code_owner_approved)),
...
never_merge_with_changes = Unreachable(
    And(PullRequest.state == PRState.MERGED, PullRequest.changes_requested),
    label="a PR can never merge with outstanding change requests",
)

never_merge_behind = Unreachable(
    And(PullRequest.state == PRState.MERGED, PullRequest.behind_base),
