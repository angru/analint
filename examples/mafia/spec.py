...
NOT fixed: the queries quantify over a declarative ``Initial`` relation
(research/16), and the actions are role-generic
...
# Quint's nondeterministic init: role fields range over the Role enum, and a
# shared predicate keeps exactly one mafia. Eight candidates become three roots.
exactly_one_mafia = Or(
    And(
        Mahtab.role == Role.MAFIA,
        Gabriela.role == Role.CITIZEN,
        Max.role == Role.CITIZEN,
    ),
    And(
        Mahtab.role == Role.CITIZEN,
        Gabriela.role == Role.MAFIA,
        Max.role == Role.CITIZEN,
    ),
    And(
        Mahtab.role == Role.CITIZEN,
...
spec = Spec(
    ...
    "the role assignment generated by a declarative initial relation",
    # The canonical model IS the set of every admissible role assignment, so the
